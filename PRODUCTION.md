...
| Upstream contribution | Twenty-eight focused branches; no upstream PR submitted | API, Runners, orchestrator, Gateway, SDK and runner branches retain their separate review boundaries and licenses; new service/reporting code is AGPL-3.0-only. Independent named-PVC `7d3238a` and deployed combined `641e2f7` remain unchanged. LLM-proxy diagnostics, the gzip test correction, volume retention/inventory fixes and three [checked-volume branches](AGYN-CHECKED-VOLUMES.md) are pushed. The checked contract is a proposal requiring coordinated dependencies, not a published API or drop-in image. Acceptance combinations are not bundled upstream proposals. Other suites retain their documented scopes; broader daemon/orchestrator race limitations remain disclosed. See [the contribution guide](CONTRIBUTING-AGYN.md). |
