...
| --- | --- | --- |
| Prepared workload activation | Combined real execution/process acceptance verified; full A2A pending | [Native and registry proposals](AGYN-PREPARED-WORKLOADS.md) persist gated execution and exact Pod/PVC bindings. [Controller migration and combined acceptance](AGYN-PREPARED-CONTROLLERS.md#combined-execution-acceptance) now pass 22 live scenarios plus three parent/group entries with real PostgreSQL, registry/native RPCs, controller-method subprocesses and Kubernetes execution. Sixteen SIGKILLs cover lost activation ACKs and removal boundaries; workspace effects survive without redispatch. Fresh controller checks pass 597 ordinary entries and 625 selected race entries with all three live fixtures enabled; the known unrelated unfiltered race/vet failures remain. Earlier native/registry component results retain their separate scopes. The workload is model-free and auth/metadata boundaries are fixtures: no full prepared-stack A2A lifecycle or permanent rollout has passed. Unknown/late prepare recovery, durable credential cleanup, zero-volume placement pins, all-writer enforcement, authentication/fencing and coordinated rollout remain open. |
...
| Existing behavior | Baseline preserved | Build and all 417 tests including subtests pass on 2026-09-14, including the 10 baseline tests, 79 read-only volume-audit tests and new prepared rollout/backup/proof coverage, on pinned Node 24.21.0 / SQLite 3.53.4. |
| Prepared rollout and registry restore | Gateway, operator subprocess tests and offline rehearsal verified; deployment pending | [Prepared rollout tooling](AGYN-PREPARED-ROLLOUT.md) stops the old orchestrator before migration and explicitly retains reviewed or partial deployments. Legacy mode rejects checked/prepared schemas instead of downgrading clients. The actual installed registry was privately dumped, restored and migrated through `0022` only in a disposable network-disabled database, with matching restore fingerprints and unchanged projected lifecycle history. Cleanup is confirmed. All 276 Gateway race entries pass; live prepared binding/absence assertions have 22 proof tests but no real-agent run yet. Installed services/data/RBAC remain unchanged; runner Secret get/patch and exact Namespace GET are missing. Image builds, controlled rollout, external-writer fencing, real agents and whole-platform backup/restore remain required. |
...
| Checked-volume upgrade | Read-only installed-state audit verified; adoption/drain pending | [The latest capture](AGYN-PREPARED-WORKLOADS.md#installed-state) at 14:56:10 UTC finds 61 legacy records, 60 matching task PVCs and 125 confirmed workloads. Migrations `0018`-`0021` are absent; stock clients remain. The audit checks backend identity and its validated database constraint; its 79 tests passed within the 323-test service suite. The 66 findings grant no adoption, deletion or rollout authority. No installed database or deployment was changed. Explicit reconciliation, all-writer draining, compatible clients, authenticated backend identity and coordinated A2A acceptance remain required. |
...
separate from the thirty-six focused contributions above. Controller source is
