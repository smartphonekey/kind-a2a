...
| --- | --- | --- |
| Prepared workload activation | Native and registry source separately verified; controller migration and combined acceptance pending | [Native runner proposals](AGYN-PREPARED-WORKLOADS.md) create gated Pods and acquire per-Pod claim holds before UID/RV-checked activation; 468 ordinary native race entries, 1,060 repeated entries and three isolated Kubernetes scenarios pass. [Registry proposals](AGYN-PREPARED-REGISTRY.md) persist immutable bindings, revision CAS and owner/backend pins with database old-writer guards; all 538 registry race tests and 2,320 repeated entries pass, including 48 blocked interleavings and non-adopting upgrade checks. Registry native observations and authorization are fixtures, not combined A2A acceptance. Both controller migrations, uncertain-prepare/late-operation reconciliation, all-writer enforcement, authentication/fencing and coordinated rollout remain open. |
...
Four additional dependent prepared-workload branches cover the
[native lifecycle](AGYN-PREPARED-WORKLOADS.md) and
[registry persistence](AGYN-PREPARED-REGISTRY.md), separate from the thirty-six
focused contributions above. They require controller integration and combined
