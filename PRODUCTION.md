...
| Checked volume deletion | Controller/native component acceptance verified; coordinated rollout pending | [Checked removal](AGYN-CHECKED-VOLUMES.md) adds immutable bindings/intents, revision CAS, old-writer database guards and physical-absence confirmation. [Caller migration `eebf4cf`](AGYN-CHECKED-VOLUMES.md#controller-migration) now routes every orchestrator/sandbox volume mutation through checked APIs, validates listings/replies and resumes only persisted deletion targets. Build and 488 ordinary tests pass; 488 selected race tests pass with native agent/sandbox cleanup and exactly the known group-consumer test excluded. Unfiltered race and vet limitations remain disclosed. Registry/Agents clients in this native fixture are fakes; combined database/controller/runner/A2A acceptance, backend routing identity, authorization, all-writer rollout, garbage collection and late-create/node fencing remain required. All 60 existing claims are unchanged; nothing is permanently deployed. |
| Workload admission and volume deletion | Registry/database contention and caller migration verified separately; combined acceptance pending | [Dependent Runners guard `f05b479`](AGYN-CHECKED-VOLUMES.md#workload-admission) serializes checked-owner admission with deletion, retains unconfirmed predecessor reservations and protects workload identity/confirmation from old SQL. Build, vet and all 335 registry race tests pass with both real PostgreSQL fixtures enabled. Forty-eight blocked interleavings cover both owner kinds, all isolation settings, rollback and competing starts while another owner progresses. Upgrade refuses contradictory history without rewriting it. The caller migration is now component-tested, but concurrent real registry/controller/runner acceptance and coordinated migrations `0017`-`0019` rollout remain required; this is not native late-create/node fencing or a new A2A lifecycle acceptance. |
...
| Upstream contribution | Thirty focused branches; no upstream PR submitted | API, Runners, orchestrator, Gateway, SDK and runner branches retain their separate review boundaries and licenses; new service/reporting code is AGPL-3.0-only. Independent named-PVC `7d3238a` and deployed combined `641e2f7` remain unchanged. LLM-proxy diagnostics, the gzip test correction, volume retention/inventory fixes, three [checked-volume branches](AGYN-CHECKED-VOLUMES.md), the dependent [owner-admission guard](AGYN-CHECKED-VOLUMES.md#workload-admission) and [checked caller migration `eebf4cf`](AGYN-CHECKED-VOLUMES.md#controller-migration) are pushed. The checked contract is a proposal requiring coordinated dependencies, not a published API or drop-in image. Acceptance combinations are not bundled upstream proposals. Other suites retain their documented scopes; broader daemon/orchestrator race and vet limitations remain disclosed. See [the contribution guide](CONTRIBUTING-AGYN.md). |
