...
| Resource containment | Per-container live bounds and shared task-count admission verified; aggregate resource gate pending | [Typed API, runner enforcement and flavor/MCP mapping](AGYN-RESOURCES.md) pass source tests and live kernel probes. The latest coordinated sweep passes completed, parallel/FIFO, interrupted, cancellation and streaming with eleven Agyn Pods having matching specs/main cgroups. [Database-wide admission](SERVICE.md#shared-execution-admission) now pins the limit, fences config drift, retains unreleased reservations and passes six-process/resize races plus delayed-release worker tests. This is not a physical-container or cluster CPU/RAM budget. Whole-task accounting, Kubernetes quota acceptance, agent OOM recovery, production sizing and mandatory production-profile enforcement remain required. Stock deployment was restored after live testing. |
...
| Upstream contribution | Seventeen focused branches; proposal drafted | API `0125665`, Runners `890f759`, orchestrator `f83ce83` and Gateway wire-test branch `6d7d432`; independent SDK diagnostics `16286f3` and stacked daemon diagnostics `2afdfc1`. Combined API/orchestrator and new SDK/daemon branches are pushed for local acceptance only. Buf compatibility, full Runners/Gateway/SDK race, real PostgreSQL and Gateway transport checks pass; daemon/orchestrator checks retain the disclosed broader race limitations. Prior daemon/SDK/resource branches retain separate evidence. Existing upstream licenses remain; new service/reporting code is AGPL-3.0-only. No upstream PR submitted yet. |
