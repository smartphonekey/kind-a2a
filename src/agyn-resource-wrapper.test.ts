...
for (const mode of ["success", "child-failure", "runner-patch-failure", "runner-rollout-failure", "orchestrator-patch-failure",
  ...
  test(`resource deployment wrapper: ${mode}`, t => {
    ...
    const original = Object.fromEntries(["k8s-runner", "agents-orchestrator"].map(name => [name, {
      metadata: { uid: `${name}-uid`, resourceVersion: "1" }, spec: { template: { spec: { containers: [{ name, image: `stock-${name}:1`, env: [
        ...
        ...(name === "k8s-runner" ? [{ name: "SUPPORTING_CONTAINER_RESOURCES", valueFrom: { configMapKeyRef: { name: "original", key: "bounds" } } }]
          : [{ name: "AGYND_CLI_INIT_IMAGE", value: "stock-init:1" }, { name: "STOP_TIMEOUT_SEC" }])
      ] }] } } }
    }]));
    const stateFile = join(directory, "state.json");
    writeFileSync(stateFile, JSON.stringify({ deployments: original, failed: false, busy: false }));
...
if(args.includes('get')) {
  ...
  if(!state.failed && ((mode==='runner-patch-failure'&&name==='k8s-runner')||(mode==='orchestrator-patch-failure'&&name==='agents-orchestrator')))fail();
  ...
  assert.equal(p[1].path,'/spec/template/spec/containers/0/image');assert.equal(p[2].path,'/spec/template/spec/containers/0/env');
  c.image=p[1].value;c.env=p[2].value;d.metadata.resourceVersion=String(Number(d.metadata.resourceVersion)+1);save();
  if(mode==='lost-runner-patch-ack'&&!state.failed&&name==='k8s-runner')fail();
} else if(args.includes('rollout')) {
  if(mode==='runner-rollout-failure'&&!state.failed&&args.includes('deployment/k8s-runner'))fail();
...
for(const name of ['k8s-runner','agents-orchestrator'])assert.equal(s.deployments[name].spec.template.spec.containers[0].image,'reviewed-'+name+':1');
...
`);
    ...
    for (const name of ["k8s-runner", "agents-orchestrator"]) {
    ...
    if (existsSync(join(directory, ".state"))) for (const entry of readdirSync(join(directory, ".state"))) {
      ...
      assert.equal(JSON.parse(readFileSync(join(path, "before.json"), "utf8")).deployments.length, 2);
    }
  });
}
