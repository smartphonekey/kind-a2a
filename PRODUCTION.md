...
The [anchored registry backup](AGYN-ANCHORED-BACKUP.md) now covers the new
ownership, revocation and retirement documents plus database guard definitions.
An actual installed-source restore and `0022`-to-`0026` migration rehearsal pass
offline, without modifying installed data. A populated-history PostgreSQL test
also restores pending recovery records and detects previously invisible document
