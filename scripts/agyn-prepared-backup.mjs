...
let directory, createAttempted = false, stage = "configuration", cleanupConfirmed = false, receipt;
...
try {
  ...
  const before = collectPreparedUpgradeState(k, scope);
  assert.equal(before.registry.workloads.unconfirmed, 0, "drain unconfirmed workloads before backup rehearsal");
  ...
  assert.equal(collectPreparedUpgradeState(k, scope).fingerprint, before.fingerprint, "source changed during backup");
  ...
  const restored = parsePreparedUpgradeState(sql(preparedUpgradeSQL), scope);
  ...
  const applied = [];
  for (const version of ["0018_checked_volume_lifecycle.sql", "0019_volume_workload_admission.sql", "0020_legacy_volume_adoption.sql", "0021_volume_backend_identity.sql", "0022_prepared_workloads.sql"]) {
  ...
  const upgraded = parsePreparedUpgradeState(sql(preparedUpgradeSQL), scope);
  ...
  assert.equal(collectPreparedUpgradeState(k, scope).fingerprint, before.fingerprint, "installed source changed during rehearsal");
  receipt = { kind: "prepared-upgrade-restored-backup", version: 1, scope, sourceFingerprint: before.fingerprint, restoredFingerprint: restored.fingerprint,
    archiveSha256: createHash("sha256").update(archive).digest("hex"), postgresImage: image, restoredAt: new Date().toISOString(),
    rehearsal: { schemaVerified: true, migrations: applied, legacyHistoryUnchanged: true }, installedDatabaseModified: false };
