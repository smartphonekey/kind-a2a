...
| --- | --- | --- |
| Prepared workload activation | Both controllers migrated and component-tested; combined A2A acceptance pending | [Native and registry proposals](AGYN-PREPARED-WORKLOADS.md) persist gated execution and exact Pod/PVC bindings. [Controller migration and inspection](AGYN-PREPARED-CONTROLLERS.md) now pass 597 ordinary controller entries, 596 selected race entries and 2,360 repeated entries; the known unrelated unfiltered race/vet failures remain. Inspection passes 493 native race entries and three isolated Kubernetes scenarios plus parent. The earlier registry's 538 race tests and 2,320 repeated entries are separate database acceptance. No new combined registry/controller/native/A2A lifecycle has passed, and nothing is permanently deployed. Unknown/late prepare recovery, durable credential cleanup, zero-volume placement pins, all-writer enforcement, authentication/fencing and coordinated rollout remain open. |
...
migrated and component-tested; combined prepared-stack acceptance and production
enforcement are required before rollout.
