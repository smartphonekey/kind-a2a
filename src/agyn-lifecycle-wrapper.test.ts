...
for (const mode of ["success", "child-failure", "patch-failure", "unmanaged-edit", "managed-edit", "busy"]) {
...
`);
    ...
    assert.equal(result.status === 0, ["success", "unmanaged-edit"].includes(mode), result.stderr);
