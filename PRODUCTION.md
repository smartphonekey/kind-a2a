...
| Agent portability | Claude completed and interrupted recovery verified; full gate pending | [Fresh Max acceptance](AGYN-PORTABILITY.md#refreshed-subscription-acceptance) passes completed-turn continuation and a separate explicit interrupted-turn recovery without changing the A2A controller/workflows. The earlier interruption failure remains unclassified. A subsequent streaming follow-up failed; the new bounded runtime observer captured `api_status=401`, `terminal_reason=api_error`, and daemon exit 1. Quarantine and cleanup worked; cancellation/parallel were not run. Credential resolution/interception versus upstream refusal still needs diagnosis. Source/native diagnostics and Codex regressions do not make this full Claude portability acceptance. |
...
| Startup reliability | Native authentication failure classified; cause still open | Earlier unexplained native/setup failures remain. A fresh Claude streaming follow-up produced a safely captured HTTP 401 result and was quarantined, with confirmed removal. Other fresh completed/interrupted passes do not explain it. The new read-only observer retains bounded Pod/exit/result metadata before cleanup; it is acceptance diagnostics, not production runtime-error storage. Investigate the credential/proxy/provider boundary before release. |
| Operations | Coordinated local rollout, restoration, retention and migration verified | 59 subprocess cases cover rollout/restoration and guards. The latest combined PVC A2A audit verifies restored stock deployment UIDs/settings/readiness, removed temporary Role/RoleBinding, unchanged ClusterRole, zero workload Pods/Services/quotas, all 55 previous claim UIDs/specs/phases and 16 older Secret identities unchanged, and 56 retained claims. Five removal confirmations survive restoration; the new instance is paused with an active persistent/no-TTL workspace. Three deliberately rejected provisioning attempts occurred before quarantine, not one; none ran an agent. Claude failure/pass audits remain separate. Production admission/load, draining, readiness, storage backup/restore, active upgrade, retention and disaster recovery remain required. |
| Upstream contribution | Twenty-one focused branches; no upstream PR submitted | Existing API, Runners, orchestrator, Gateway, SDK and runner branches retain their separate review boundaries and licenses; new service/reporting code is AGPL-3.0-only. Independent named-PVC `7d3238a` is unchanged. Combined `lab/pvc-owner-integration` (`641e2f7`) is pushed and now passes native Kubernetes plus deployed A2A recovery. These acceptance branches are not bundled upstream proposals. Full service, Runners/Gateway/SDK/runner race and database/native checks pass in their documented scopes; broader daemon/orchestrator race limitations remain disclosed. See [the contribution guide](CONTRIBUTING-AGYN.md). |
