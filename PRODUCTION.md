...
| First-provision cleanup | Focused runner fix and native quota matrix verified; coordinated deployment pending | [Startup-secret fix `fcd7cf6`](AGYN-RESOURCES.md#native-first-provision-failures) preserves durable PVCs, cleans up owned credentials after rejection and retains them after uncertain Pod creation. Published API generation, build and 146 runner race-suite tests pass. Seven real quota stages and explicit partial-PVC continuation pass with zero Pod admission; all 49 prior PVCs and four stock deployments are unchanged. This fix is not yet in the coordinated A2A images. First-provision A2A recovery, named-PVC identity enforcement, crash-orphan reconciliation, late-create fencing and post-success Stop/Remove cleanup remain required. |
