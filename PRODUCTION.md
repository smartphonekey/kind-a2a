...
| --- | --- | --- |
| Prepared workload activation | Native source and isolated Kubernetes acceptance verified; registry/controller migration pending | [Dependent API and native runner proposals](AGYN-PREPARED-WORKLOADS.md) create gated Pods and acquire per-Pod claim holds before UID/RV-checked activation. All 468 ordinary native race entries and 1,060 repeated prepared entries pass; three live scenarios verify real execution/resume, PVC delete exclusion and stale Pod activation rejection. Temporary Secrets are Pod-UID-owned and GC-tested. Immutable binding storage, both controller migrations, late-operation reconciliation, all-writer enforcement, authentication/fencing and coordinated A2A rollout remain open. |
...
Two additional dependent [prepared-workload branches](AGYN-PREPARED-WORKLOADS.md)
are built on acceptance combinations, separate from the thirty-six focused
contributions above. They require control-plane integration before rollout.
