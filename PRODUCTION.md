...
| Volume ownership | Closed-record path verified; production gate pending | [Runners fix and acceptance](AGYN-RESOURCES.md#closed-volume-ownership) preserve the complete identity tuple in an atomic reopen update. Tests reproduce 22 foreign-identity writes and a concurrent takeover against old code; fixed source, real PostgreSQL contention and combined first-provision A2A recovery pass with the actual new binary verified. Same-owner recovery and native session continuity survive Pod replacement. Named-PVC reuse and sandbox open-record validation remain unguarded; authentication, infrastructure fencing and upgrade of all writers are still required. Stock Runners is restored, not permanently fixed. |
