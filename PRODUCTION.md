...
| Operations | Local four-component rollout/restoration and migration verified | 59 subprocess cases cover dependency order/restoration, failures, conflicts, identity changes, busy-lab refusal, model-free preflight, quota and Secret-read guards. The latest first-PVC audit independently verified restored stock settings/UIDs/rollouts, removed temporary Role/RoleBinding, unchanged ClusterRole, zero workload Pods/Services/quotas, all 49 prior PVCs unchanged and 50 retained total. Its three workload confirmations survived image downgrade; the new instance is paused with an active persistent/no-TTL workspace. Earlier quota, combined-runtime and eleven-workload evidence remains separate. A private pre-migration archive exists, but archive listing is not a restore test. Production admission/load, graceful draining, readiness, backup/restore, active upgrade, retention and disaster recovery remain required. |
