...
| Checked-volume upgrade | Read-only installed-state audit verified; adoption/drain pending | [The audit](AGYN-VOLUME-BACKEND.md#installed-state-and-remaining-work) now checks backend identity and the validated database constraint. The latest 14:08:17 UTC capture finds 61 legacy records, 60 matching task PVCs and 125 confirmed workloads. Migrations `0018`-`0021` are absent; stock clients remain. Its 79 tests pass within the 323-test service suite. The 66 findings grant no adoption, deletion or rollout authority. No installed database or deployment was changed. Explicit reconciliation, all-writer draining, compatible clients, authenticated backend identity and coordinated A2A acceptance remain required. |
