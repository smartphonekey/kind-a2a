...
| --- | --- | --- |
| Existing behavior | Baseline preserved | Build and all 186 top-level tests pass on 2026-09-14 (205 including subtests), including the 10 baseline tests, on pinned Node 24.21.0 / SQLite 3.53.4. |
...
| Resource containment | Per-container bounds, shared task-count admission and local A2A quota recovery verified; production gate pending | [Typed API, runner enforcement and flavor/MCP mapping](AGYN-RESOURCES.md) pass source tests, kernel probes and the coordinated five-scenario Codex sweep. [Database-wide admission](SERVICE.md#shared-execution-admission) pins the limit and retains unreleased reservations. [Native quota acceptance](AGYN-RESOURCES.md#native-namespace-quota-acceptance) covers effective init/sidecar accounting and competing starts. The new [A2A rejection/recovery scenario](AGYN-RESOURCES.md#a2a-quota-recovery) also passes: one persisted inbox receipt, no rejected Pod, explicit confirmation before settlement, no retry when capacity returns, same session/PVC after owner reconciliation and acknowledgement-only retirement. Coordinated first-provision A2A recovery, whole-task cost reporting, agent OOM recovery, sizing, protected quota bootstrap and mandatory production profiles remain required. Stock deployments are restored with no permanent workload quota. |
| First-provision cleanup | Focused native matrix and combined source validation pass; coordinated deployment pending | [Startup-secret fix `fcd7cf6`](AGYN-RESOURCES.md#native-first-provision-failures) preserves durable PVCs, cleans up owned credentials after rejection and retains them after uncertain Pod creation. Published API generation, build and 146 runner race-suite tests pass. Seven real quota stages and explicit partial-PVC continuation pass with zero Pod admission; all 49 prior PVCs and four stock deployments are unchanged. Combined source `74faf0e` also passes local API generation, build and the full race suite, but is not yet in the coordinated A2A images. First-provision A2A recovery, named-PVC identity enforcement, crash-orphan reconciliation, late-create fencing and post-success Stop/Remove cleanup remain required. |
...
| Operations | Local four-component rollout/restoration and migration verified | 50 subprocess cases cover dependency order/restoration, failures, conflicts, identity changes, busy-lab refusal, model-free preflight and quota guards. The new quota-recovery audit independently verified restored stock settings/UIDs/rollouts, zero workload Pods/Services/quotas, all 48 prior PVCs unchanged and 49 retained total. Its three workload confirmations survived image downgrade; the new instance is paused with an active persistent/no-TTL workspace. Earlier combined-runtime and eleven-workload evidence remains separate. A private pre-migration archive exists, but archive listing is not a restore test. Production admission/load, graceful draining, readiness, backup/restore, active upgrade, retention and disaster recovery remain required. |
| Upstream contribution | Nineteen focused branches; proposal drafted | API `0125665`, Runners `890f759`, orchestrator `f83ce83` and Gateway wire-test branch `6d7d432`; independent SDK diagnostics `16286f3` and stacked daemon diagnostics `2afdfc1`. Runner chart `28d4562` adds native namespace quota; independent runner fix `fcd7cf6` adds attempt-scoped startup-secret cleanup. Combined branches are for local acceptance only. Buf compatibility, full Runners/Gateway/SDK/runner race, real PostgreSQL, Gateway transport and controlled runner/Kubernetes quota checks pass; daemon/orchestrator checks retain the disclosed broader race limitations. Prior branches retain separate evidence. Existing upstream licenses remain; new service/reporting code is AGPL-3.0-only. No upstream PR submitted yet. |
