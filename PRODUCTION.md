...
| Checked volume deletion | API/registry/native acceptance verified; controller integration pending | [Checked removal](AGYN-CHECKED-VOLUMES.md) adds immutable bindings/intents, revision CAS, old-writer database guards, physical-absence confirmation and fail-closed legacy runner paths. Focused and combined race/database suites pass; real Kubernetes rejects both replacement-UID and same-UID ownership-change races with HTTP 409. All 60 existing claims are unchanged. Every orchestrator/sandbox caller still needs migration; backend routing identity, authorization, all-writer rollout, end-to-end acceptance, garbage collection and late-create/node fencing remain required. Nothing is permanently deployed. |
| Workload admission and volume deletion | Registry/database contention and upgrade verified; integration pending | [Dependent Runners guard `f05b479`](AGYN-CHECKED-VOLUMES.md#workload-admission) serializes checked-owner admission with deletion, retains unconfirmed predecessor reservations and protects workload identity/confirmation from old SQL. Build, vet and all 335 race tests pass with both real PostgreSQL fixtures enabled. Forty-eight blocked interleavings cover both owner kinds, all isolation settings, rollback and competing starts while another owner progresses. Upgrade refuses contradictory history without rewriting it. Controller/sandbox migration and coordinated migrations `0017`-`0019` rollout remain required; this is not native late-create/node fencing or a new A2A lifecycle acceptance. |
...
| Upstream contribution | Twenty-nine focused branches; no upstream PR submitted | API, Runners, orchestrator, Gateway, SDK and runner branches retain their separate review boundaries and licenses; new service/reporting code is AGPL-3.0-only. Independent named-PVC `7d3238a` and deployed combined `641e2f7` remain unchanged. LLM-proxy diagnostics, the gzip test correction, volume retention/inventory fixes, three [checked-volume branches](AGYN-CHECKED-VOLUMES.md) and the dependent [owner-admission guard](AGYN-CHECKED-VOLUMES.md#workload-admission) are pushed. The checked contract is a proposal requiring coordinated dependencies, not a published API or drop-in image. Acceptance combinations are not bundled upstream proposals. Other suites retain their documented scopes; broader daemon/orchestrator race limitations remain disclosed. See [the contribution guide](CONTRIBUTING-AGYN.md). |
