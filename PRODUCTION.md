...
| --- | --- | --- |
| Existing behavior | Baseline preserved | Build and all 308 tests including subtests pass on 2026-09-14, including the 10 baseline tests and 64 new read-only upgrade audit tests, on pinned Node 24.21.0 / SQLite 3.53.4. |
...
| Checked-volume upgrade | Read-only installed-state audit verified; adoption/drain pending | [The audit command](AGYN-CHECKED-VOLUMES.md#read-only-upgrade-audit) uses a read-only MVCC registry snapshot, before/after Kubernetes inventories, pinned namespace/PostgreSQL identities and private reports. Two live captures find 61 legacy records: 60 active records match retained PVC ownership, one failed unbound record has no physical match. All 125 workloads are confirmed; migrations `0018`/`0019` are absent and stock clients remain. Its 64 tests pass within the 308-test service suite. No data or deployment changes were made. Findings are not rollout/adoption/deletion authority; explicit legacy reconciliation, full writer draining, compatible clients, backend fencing and coordinated A2A acceptance remain required. |
