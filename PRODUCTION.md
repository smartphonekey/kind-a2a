...
| --- | --- | --- |
| Existing behavior | Baseline preserved | Build and all 171 top-level tests pass on 2026-09-14 (190 including subtests), including the 10 baseline tests, on pinned Node 24.21.0 / SQLite 3.53.4. |
...
| Resource containment | Per-container bounds, shared task-count admission and controlled native quota verified; production gate pending | [Typed API, runner enforcement and flavor/MCP mapping](AGYN-RESOURCES.md) pass source tests and live kernel probes. The coordinated sweep passes completed, parallel/FIFO, interrupted, cancellation and streaming with eleven Agyn Pods having matching specs/main cgroups. [Database-wide admission](SERVICE.md#shared-execution-admission) pins the limit, fences config drift and retains unreleased reservations. The separate [native quota fixture](AGYN-RESOURCES.md#native-namespace-quota-acceptance) now passes independent CPU/memory rejection, effective init/sidecar accounting, retained-Pod counting, six competing starts and observed release. This is not deployed A2A quota recovery or a budget across all namespaces. Whole-task cost reporting, agent OOM recovery, production sizing, protected quota bootstrap and mandatory profiles remain required. Stock deployments remain unchanged with no installed workload quota. |
...
| Operations | Local four-component rollout/restoration and migration verified | 41 subprocess cases cover dependency order/restoration, failures, conflicts, identity changes, busy-lab refusal and model-free scenario preflight. The latest combined-runtime audit independently verified restored stock settings/UIDs/rollouts, zero workload Pods/Services and 48 retained PVCs, including all 46 prior UIDs/phases unchanged. All five new workload confirmations survived image downgrade; both new instances are paused with persistent/no-TTL workspace definitions. Earlier eleven-workload evidence remains recorded separately. A private pre-migration archive exists, but archive listing is not a restore test. Production admission/load, graceful draining, readiness, backup/restore, active upgrade, retention and disaster recovery remain required. |
| Upstream contribution | Eighteen focused branches; proposal drafted | API `0125665`, Runners `890f759`, orchestrator `f83ce83` and Gateway wire-test branch `6d7d432`; independent SDK diagnostics `16286f3` and stacked daemon diagnostics `2afdfc1`. New runner chart `28d4562` adds native namespace quota, independently of the resource API. Combined branches are for local acceptance only. Buf compatibility, full Runners/Gateway/SDK/runner race, real PostgreSQL, Gateway transport and controlled runner/Kubernetes quota checks pass; daemon/orchestrator checks retain the disclosed broader race limitations. Prior branches retain separate evidence. Existing upstream licenses remain; new service/reporting code is AGPL-3.0-only. No upstream PR submitted yet. |
