...
| Upstream contribution | Twenty-five focused branches; no upstream PR submitted | API, Runners, orchestrator, Gateway, SDK and runner branches retain their separate review boundaries and licenses; new service/reporting code is AGPL-3.0-only. Independent named-PVC `7d3238a` and deployed combined `641e2f7` remain unchanged. LLM-proxy diagnostics `8abd410`, the independent gzip test correction `c8f2c51`, and the two [volume retention/inventory fixes](AGYN-VOLUME-SAFETY.md) are pushed. Acceptance combinations are not bundled upstream proposals. Other suites retain their documented scopes; broader daemon/orchestrator race limitations remain disclosed. See [the contribution guide](CONTRIBUTING-AGYN.md). |
